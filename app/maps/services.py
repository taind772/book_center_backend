...
class DocumentLabelMap:
    ...
    def dl_get_documents(label_uuid: uuid.UUID):
        document_uuid_list = DocumentLabel.objects.filter(first_uuid=label_uuid).values_list('document_uuid', flat=True)
...
class DocumentAuthorMap:
    ...
    def da_get_documents(author_uuid: uuid.UUID):
        document_uuid_list = DocumentAuthor.objects.filter(second_uuid=author_uuid).values_list('document_uuid', flat=True)
        return DocumentServices.document_get_all(document_uuid_list=document_uuid_list)

    @staticmethod
    def da_get_authors(document_uuid: uuid.UUID):
        author_uuid_list = DocumentAuthor.objects.filter(first_uuid=document_uuid).values_list('group_uuid', flat=True)
