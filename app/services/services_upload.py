...
from maps.services import DocumentAuthorMap, DocumentPublisherMap, DocumentLabelMap
from .services_drive import DriveServices
...
def upload(info, title: str, description: str, release_year: int, language: str, category: str,
           authors_name: str, publishers: str, labels: str, file, cover_url: str) -> Upload:
    ...
    if labels is not None:
        for lb_name in set((_.strip() for _ in labels.split(','))):
            ...
            DocumentLabelMap.dl_create(document_uuid=document.document_uuid, label_uuid=lb.id)
